...
そこで「消えると決まった塊」は `ClearScheduler` が盤面の外で覚えておき、
一度印がついたら窓がどこにあろうと最後まで一緒に消えるようにした。
セルに持たせたカウンタは、点滅を描くための写しでしかない。

覚えるのは座標だけでは足りなかった。点滅は 0.5 秒ほど続くので、その間に
プレイヤーは点滅中のブロックを掘れる。座標だけを見ていると、
**空いた穴に落ちてきた無関係なブロックを塊の一部として消してしまう**。
「その時そこにあったセル」まで覚えて、消す直前に本人か確かめている。
...
列で絞るだけでも足りない。同じ列の 9 行離れた場所で偶然続いた消去まで連鎖になる。

そこで**消した跡のマスそのもの**を覚え、そこへ落ちてきたブロックに印を引き継ぐ。
連鎖と数えるのは、その印を持つブロックが関わった消去だけ。
落下は 1 ティックに 1 マスなので、「移動前のマスが跡なら、移動後も跡の続き」と
辿っていけば、掘って崩した結果がどこへ落ちたかを追える。

条件は連鎖を**切る側だけでなく数える側にも**いる。消えるたびに無条件で足していると、
前の消去が片付く前に離れた場所が消えただけで連鎖になってしまう。
